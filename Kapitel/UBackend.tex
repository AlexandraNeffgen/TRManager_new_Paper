...
Hibernate(Hibernate ist ein externes Tool und gehört nicht direkt zu Spring),
die den Zugriff auf der Datenbank stark vereinfacht und Änderungen an der
...
\subsection{Implementierung}
	...
	den Anwendungsentwickler Funktionen wie Dependency Injection und ähnliche
	...
	Die für dieses Projekt verwendeten Module umfassen Spring Boot,Spring Data und
	Spring MVC.
	Spring Boot erlaubt die einfache Erstellung sofort lauffähiger Spring-Anwendungen, da hier keine komplexen
	...
	\subsubsection{Model}\label{sssec:model}
		...
		Anwendung dar wie zum Beispiel eine Klasse oder einen Schüler. Ebenso legen
		diese über die annotierten Model-Klassen die Datenbankstruktur fest (Siehe Kapitel "`Hibernate"'). Für die Darstellung der Daten werden
		...
		den schickenden und den beaufsichtigenden Lehrer, den zugehörigen Schüler,
	...
	Queries zuständig und liefert Daten zurück und schreibt Daten in die
	...
	möglich, andere Queries als die von Spring generierten zu verwenden, zum
	Beispiel falls die per Standarad gegebenen Zugriffsmöglichkeiten von Spring
	...
	Repositories für die Form-Objekte. Das Repository ist im Grunde ein Interface, das von
	...
	Abstraktion der Repositories für die restliche Anwendung und behandeln Ausnahmefälle, damit diese nicht an die
	...
	beziehungsweise in der Geschäftslogik umgesetzt werden sollen) sowie die
	...
	implementiert, der das clientseitige Backup unterstützt (alle Daten
	ausliefern, Daten löschen und die Tabellen in den Anfangszustand versetzen)
...
implementierten Logik zurückgeben sowie richtig in die Datenbank übernehmen.
	...
	"`erzwungen"'. Im Fall dieser Anwendung gibt es für jeden Controller zwei
	Testfäll, entweder "`Not found"' beziehungsweise "`Internal Server Error"' oder
	"`OK"'.\\
	...
	die anderen Schichten mitgetestet werden.
	 ...
	 \paragraph{Teacher-Controller}\label{ssssec:test_tcontroller}$~~$\\
			...
			zurückgegeben werden, falls der Datensatz nicht vorhanden und umgekehrt für
			...
			wird. Für den hier getesteten Fehler würden eventuell andere Statuscodes
			besser passen wie zum Beispiel "`Unprocessable Entity"', der dem
			Anwendungsentwickler mehr Aufschluss geben würde.\\
